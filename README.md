...
Node.js is a back-end runtime environment that executes JavaScript code outside of a web browser. It is mainly used for creating servers and backend API services. Express is a backend web application framework for Node.js for building web applications and APIs. Its main use is to provide server-side logic for web apps.

1. Understand and explain the use of Middleware.

Middleware are functions that have access to the request object, response object, and the next middleware function. The next function allows the next middleware function to be executed after the current one.

1. The basic principles of the REST architectural style.

Representational State Transfer (REST) is a software architectural style that was created to guide the design for the World Wide Web. REST defines a set of constraints that allows APIs to interact with RESTful web services. It involves a client, server, and interactions between them called HTTP requests and responses.

1. Understand and explain the use of Express Routers.

Express Routers are a way to create route handlers. It is mainly used to organize and maintain code. Routes can be separated in different files to help organize every middleware function in a web application.

1. Describe tooling used to manually test the correctness of an API.

There are many different tools to manually test the correctness of an API. Postman, Insomnia, HTTPie are good examples of applications that can request information from a server with an API. GET, POST, PUT, and DELETE are examples of the request methods used for these applications to send requests to different routes for specific pieces of data. A fully functional API server has the ability to perform CRUD operations which allow users to Create, Read, Update, and Delete data.
